...
public class ShowPath
{
	...
	private static String getAllPaths(final BinaryNodeInterface<Character> root)
	{
		// TO DO
		String path = ""; 
		return getAllPaths(root, path); 
	}
	private static String getAllPaths(final BinaryNodeInterface<Character> root, String path)
	{
		String result = "" + root.getData() + " " + path + "\n"; 
		//StringBuilder sbPath = new StringBuilder(); 
		if(root.hasLeftChild())
		{
			result += getAllPaths(root.getLeftChild(), path.concat("0").toString()); 
		}
		if(root.hasRightChild())
		{
			result += getAllPaths(root.getRightChild(), path.concat("1").toString()); 
		}
		return result; 
	}
